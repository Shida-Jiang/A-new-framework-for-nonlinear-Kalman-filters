...
for i=scale
    ...
    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,0,0);
    xplotEKF_1=[xplotEKF_1 SOC_rmse];
    yplotEKF_1=[yplotEKF_1 SOH_rmse];
    Time_EKF_1=[Time_EKF_1 Runtime];
    freq_back_EKF_1=[freq_back_EKF_1 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,0,1);
    xplotEKF_2=[xplotEKF_2 SOC_rmse];
    yplotEKF_2=[yplotEKF_2 SOH_rmse];
    Time_EKF_2=[Time_EKF_2 Runtime];
    freq_back_EKF_2=[freq_back_EKF_2 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,3,0);
    xplotEKF2_1=[xplotEKF2_1 SOC_rmse];
    yplotEKF2_1=[yplotEKF2_1 SOH_rmse];
    Time_EKF2_1=[Time_EKF2_1 Runtime];
    freq_back_EKF2_1=[freq_back_EKF2_1 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,3,1);
    xplotEKF2_2=[xplotEKF2_2 SOC_rmse];
    yplotEKF2_2=[yplotEKF2_2 SOH_rmse];
    Time_EKF2_2=[Time_EKF2_2 Runtime];
    freq_back_EKF2_2=[freq_back_EKF2_2 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,1,0);
    xplotUKF_1=[xplotUKF_1 SOC_rmse];
    yplotUKF_1=[yplotUKF_1 SOH_rmse];
    Time_UKF_1=[Time_UKF_1 Runtime];
    freq_back_UKF_1=[freq_back_UKF_1 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,1,1);
    xplotUKF_2=[xplotUKF_2 SOC_rmse];
    yplotUKF_2=[yplotUKF_2 SOH_rmse];
    Time_UKF_2=[Time_UKF_2 Runtime];
    freq_back_UKF_2=[freq_back_UKF_2 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,2,0);
    xplotCKF_1=[xplotCKF_1 SOC_rmse];
    yplotCKF_1=[yplotCKF_1 SOH_rmse];
    Time_CKF_1=[Time_CKF_1 Runtime];
    freq_back_CKF_1=[freq_back_CKF_1 freq_back];

    [SOC_rmse,SOH_rmse,V_rmse, Runtime, freq_back]=simulation(1,magnitude,repeat,2,1);
    xplotCKF_2=[xplotCKF_2 SOC_rmse];
    yplotCKF_2=[yplotCKF_2 SOH_rmse];
    Time_CKF_2=[Time_CKF_2 Runtime];
    freq_back_CKF_2=[freq_back_CKF_2 freq_back];
end
%% display Runtime
...
xlim([10^-5 1])
exportgraphics(f1,'SOC-SOH-no-back-out.png','Resolution',900)
%%
f3=figure('Position',[100 100 450 300]);

hold on

h1_2=plot(noise1_ref*10.^scale, freq_back_EKF_2*100, '-o', 'DisplayName', "EKF", 'Color', '#0072BD','LineWidth',1);
h2_2=plot(noise1_ref*10.^scale, freq_back_EKF2_2*100, '-s', 'DisplayName', "EKF2", 'Color', "#D95319",'LineWidth',1);
h3_2=plot(noise1_ref*10.^scale, freq_back_UKF_2*100, '-^', 'DisplayName', "UKF", 'Color', "#EDB120",'LineWidth',1);
h4_2=plot(noise1_ref*10.^scale, freq_back_CKF_2*100, '-x', 'DisplayName', "CKF", 'Color', "#7E2F8E",'LineWidth',1);
...
exportgraphics(f3,'battery_backout_freq.png','Resolution',900)
function [SOC_rmse,SOH_rmse,V_rmse,Runtime,freq_back]=simulation(improvement1,noise,repeat,KFtype,backout)
%% Problem setup
    ...
    for i=1:stepnum
        if(KFtype==0)
            ...
            if(improvement1==1)
                ...
                if(sum(diag(temp))<sum(diag(Variance)) || backout==0)
                    Variance=temp;
                else
                    states=states0;
                    freq_back=freq_back+1/repeat/stepnum;
                end
            else
        ...
        elseif(KFtype==0.5)
            ...
            states_est(:,i+1)=states;
        elseif(KFtype==1)
            if trace(Variance)>1e10
                Variance=diag([1e8 1e8 1e8]);
            end
            L=chol(Variance).';
            state=states_est(:,i);
            n=3;
