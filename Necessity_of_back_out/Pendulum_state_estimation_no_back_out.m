...
set(gca, 'XTickLabel', []);
ylim([1e-8 10000])
set(gca, 'YTick', [1e-8 1e-4 1 10000]);
grid on
nexttile
hold on
...
set(gca, 'YTick', [1e-8 1e-4 1 10000]);
grid on
exportgraphics(f1,'pendulum_no_backout.png','Resolution',900)
%%
...
exportgraphics(f3,'pendulum_backout_freq.png','Resolution',900)

function [sitadot_rmse,sita_rmse,backout_freq]=simulation(backout,noise1,repeat,KF_type)
%% Problem setup
improvement1=1;
delta_t=0.01;
len=1;
...
for iii=1:repeat
    ...
    tic;
    for i=1:stepnum
        if trace(Variance)>1e8
            Variance=diag(ones(1,2)*1e6);
        end
        if Variance(2,2)<0
            Variance(2,2)=abs(Variance(2,2));
        end
        if(KF_type==0)
            states=states_est(:,i);
