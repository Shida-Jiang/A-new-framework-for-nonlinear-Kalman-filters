...
for i=scale
    ...
    [~,x_RMSE,backout_freq]=simulation(1,magnitude,repeat,0.5);
    len=length(x_RMSE(1,:));
    ...
    [~,x_RMSE,backout_freq]=simulation(0,magnitude,repeat,3);
    ...
    [~,x_RMSE,backout_freq]=simulation(1,magnitude,repeat,3);
    ...
    [~,x_RMSE,backout_freq]=simulation(0,magnitude,repeat,3.5);
    ...
    [~,x_RMSE,backout_freq]=simulation(1,magnitude,repeat,3.5);
...
grid on
exportgraphics(f1,'navigation_no_backout.png','Resolution',900)
%%
...
exportgraphics(f3,'navigation_backout_freq.png','Resolution',900)
...
function [time,x_RMSE,backout_freq]=simulation(backout,magnitude,M,KFtype)
rng(123)
improvement=1;
        ...
        if(KFtype==0)
            ...
            if(improvement==1)
                ...
                if(sum(diag(temp))>sum(diag(P)) && backout)
                    x_est(:,k)=x_est0;
                    backout_freq=backout_freq+1/N/M;
                else
                    P=temp;
                end
            else
